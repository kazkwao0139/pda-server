...
// 이동 후 도로 위 보정
function moveOnRoad(p, newX, newY) {
...
// 리콜 취소 → 채널링 보호막 체크 → HP 감소 → 사망 처리
function applyDamageToPlayer(attacker, target, finalDamage) {
    // 리콜 취소
    if (target.recalling) {
...
// 대시 실행 + 도로 보정 + 이펙트
function executeDash(p, dirX, dirY) {
...
// 궁극기 데미지 루프 + 이펙트
function executeUltimate(p, ultX, ultY) {
    const spec = WEAPON_SPECS[p.weaponType];
    ...
    for (const enemy of game.players) {
        ...
        if (distance({x: ultX, y: ultY}, enemy) < spec.ultRadius) {
            ...
            applyDamageToPlayer(p, enemy, finalDamage);
        }
    }
...
function attack(p, target) {
    ...
    applyDamageToPlayer(p, target, finalDamage);
...
function updatePlayer(p, dt) {
    ...
    if (input.e && p.dashCooldown <= 0) {
        ...
        executeDash(p, mouseWorldX - p.x, mouseWorldY - p.y);
        p.input.e = false;
        return;
    }
